---
title: 'Tidy Tuesday #6'
author: "Jocelyn Martinez Rico"
date: "3/22/2022"
output: 
  html_document:
    theme: yeti
    toc: TRUE
    toc_float: TRUE
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE, message = FALSE, warning = FALSE)
```

# Introduction

Tidy Tuesday Assignment #6 on baby names using data from 2022-03-22

# Load Libraries
```{r}
library(tidyverse)
library(here)
library(showtext)
library(ggplot2)
library(dplyr)
library(tidyr)
library(patchwork)
library(beyonce)
```

# Load data 
```{r}
baby_names <- read_csv(here("Tidy_Tuesday_6/data","babynames.csv"))

glimpse(baby_names)
```

# Data wrangling
```{r}
baby_names %>%
  count(year, sort = TRUE) %>%
  arrange(desc(year))

two_baby <- baby_names %>%
  filter(year >= 2000) %>% #selects year
  rename(record = n) #renames column

glimpse(two_baby)
view(two_baby)
```

## Data for girl plot
```{r}
baby_girl <- two_baby %>%
  filter(sex == "F") %>% # filters for female
  slice_max(record, n = 50) # filters for top 50 uses in data set

glimpse(baby_girl)
```

## Data for boy plot
```{r}
baby_boy <- two_baby %>%
  filter(sex =="M") %>% # filters for male
  slice_max(record, n = 50) # filters top 50 uses in data set

glimpse(baby_boy)
```

# Plotting the data
```{r, echo=FALSE}
font_add(family = "baskerville", regular = "Baskerville.ttc")
showtext_auto()
```

## Plot for baby girl names
```{r}
gcp <- c("#BC5163","#79201E","#A48CA6","#F4BADF","#E777AF","#9D204B","#F59B6C","#EA5768","#FC46AA")

bg <- baby_girl %>%
  ggplot(aes(x = year,
             y = record,
             fill = name,
             color = name))+
  geom_point(size = 4)+
  scale_x_continuous(breaks = seq(2000, 2010, 1),
                     limits = c(2000, 2010))+
  labs(x = "Year",
       y = "Name Count",
       fill = "Girl Names")+
  theme_light()+
  theme(text = element_text(family = "baskerville"),
        axis.text.x = element_text(angle = 45, hjust = 1))+
  scale_color_manual("Girl Names", values = gcp)
bg
```

## Plot for baby boy names
```{r}
bcp <- c("#6781a8","#c98366","#3d4763","#d4b6a6","#c1b85a","#627a61","#4368b4","#7f170d","#75ad5e","#212523","#996640")

bb <- baby_boy %>%
  ggplot(aes(x = year,
             y = record,
             fill = name,
             color = name))+
  geom_point(size = 4)+
  scale_x_continuous(breaks = seq(2000, 2010, 1),
                     limits = c(2000, 2010))+
  labs(x = "Year",
       y = "Name Count",
       fill = "Boy Names")+
  theme_light()+
  theme(text = element_text(family = "baskerville"),
        axis.text.x = element_text(angle = 45, hjust = 1))+
  scale_color_manual(name = "Boy Names", values = bcp)
bb
```

## Plot for most used baby names
```{r}
bg+bb+
  plot_annotation(
  title = 'Most Used Baby Names From 2000-2010',
  caption = 'Source: Tidy Tuesday Data 2022-03-22 / US Baby Names') &
  theme(plot.title = element_text(family = "baskerville"),
        plot.caption = element_text(family = "baskerville"))

ggsave(here("Tidy_Tuesday_6/output","babynames.png"), height = 6, width = 10)
```

